...
const props = defineProps<{ category: ExpenseCategory; currentRole: number; isEmpty?: boolean }>()
...
onMounted(async () => {
  if (props.isEmpty || !props.category?.id) return
  const res = await fetch(`/api/expense-categories.php?check_relations=1&id=${props.category.id}`, { credentials: 'include' })
...
async function onDeleteConfirm() {
  ...
  const res = await fetch(`/api/expense-categories.php?action=delete&id=${props.category.id}`, { method: 'DELETE', credentials: 'include' })
...
<template>
  <ConfirmModal
    ...
    :message="`Удалить категорию «${category.name}»? Это действие нельзя отменить.`"
  ...
  <Teleport to="body">
    <div class="modal-overlay-main">
      <div class="padGlass modal-sm">
        <div class="modal-glassTitle">{{ isEmpty ? 'Категории расходов' : category.name }}</div>
        <div class="ButtonFooter PosColumn">
          ...
          <button v-if="canFull || props.isEmpty" class="btnGlass iconTextStart" @click="$emit('add')">
            <span class="inner-glow"></span><span class="top-shine"></span>
            <svg-icon type="mdi" :path="mdiAccountPlusOutline" class="btn-icon" /><span>Добавить категорию</span>
          </button>
