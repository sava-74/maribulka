...
const props = defineProps<{ shootingType: ShootingType; currentRole: number; isEmpty?: boolean }>()
...
onMounted(async () => {
  if (props.isEmpty || !props.shootingType?.id) return
  const res = await fetch(`/api/shooting-types.php?check_relations=1&id=${props.shootingType.id}`, { credentials: 'include' })
...
async function onDeleteConfirm() {
  ...
  const res = await fetch(`/api/shooting-types.php?action=delete&id=${props.shootingType.id}`, { method: 'DELETE', credentials: 'include' })
...
<template>
  <ConfirmModal
    ...
    title="Удалить тип съёмки"
    :message="`Удалить «${shootingType.name}»? Это действие нельзя отменить.`"
  ...
  <Teleport to="body">
    <div class="modal-overlay-main">
      <div class="padGlass modal-sm">
        <div class="modal-glassTitle">{{ isEmpty ? 'Типы съёмок' : shootingType.name }}</div>
        <div class="ButtonFooter PosColumn">
          ...
          <button v-if="canFull || props.isEmpty" class="btnGlass iconTextStart" @click="$emit('add')">
            <span class="inner-glow"></span><span class="top-shine"></span>
            <svg-icon type="mdi" :path="mdiAccountPlusOutline" class="btn-icon" /><span>Добавить тип съёмки</span>
          </button>
