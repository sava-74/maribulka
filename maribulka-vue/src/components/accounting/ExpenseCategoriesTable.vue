...
const columns: ColumnDef<any>[] = [
  ...
  },
  {
    ...
    header: 'Статус',
    cell: ({ getValue }) => {
      const isActive = getValue() as number
...
const table = useVueTable({
  get data() {
    return referencesStore.expenseCategories
  },
  columns,
  state: {
    get sorting() {
...
const selectedExpenseCategory = computed(() => {
  return selectedExpenseCategories.value.length === 1 ? selectedExpenseCategories.value[0] : null
})
...
function applyFilters() {
  ...
  if (statusFilter.value) {
    filters.push({ id: 'is_active', value: statusFilter.value })
  }
...
async function handleDelete() {
  if (!hasSelectedRow.value || !selectedExpenseCategory.value) return

  const category = selectedExpenseCategory.value
  const hasRelations = await referencesStore.checkExpenseCategoryRelations(category.id)

  console.log('ID категории:', category.id, 'Ответ API:', hasRelations, 'Тип:', typeof hasRelations)

  if (hasRelations) {
    alertTitle.value = 'Удаление невозможно'
    alertMessage.value = `Категорию "${category.name}" удалить нельзя. С этой категорией связаны другие документы.`
...
async function confirmDelete() {
  ...
  const result = await referencesStore.deleteExpenseCategory(selectedExpenseCategory.value.id)
...
<template>
  <div class="table-general">
    ...
    <!-- Таблица -->
    <div v-if="referencesStore.expenseCategories.length > 0" class="table-scroll-container">
    ...
    <div v-else class="empty-state">
      <p>📭 Нет категорий расходов в базе</p>
    </div>
    ...
    <ConfirmModal
      ...
      :message="`Вы хотите удалить категорию &quot;${selectedExpenseCategory?.name}&quot;?`"
