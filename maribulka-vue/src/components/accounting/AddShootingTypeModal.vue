...
const handleSubmit = async () => {
  ...
  const result = await referencesStore.createShootingType({
  ...
  if (result.success) {
    ...
    alertTitle.value = 'Ошибка'
    alertMessage.value = result.error || 'Не удалось создать тип съёмки'
    showAlert.value = true
  }
}
</script>
  ...
  <Teleport to="body">
    <div v-if="isVisible" class="modal-overlay" @click.self="emit('close')">
      <div class="modal-glass">
        <div class="modal-glassTitle">Добавить тип съёмки</div>
